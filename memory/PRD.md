...
## What's been implemented (2026-02 / iteration 23 — ADMIN POLISH + TI DODO SAGA VOICE)
- ✅ **Admin Polish (P3) — complete frontend wiring**. The backend `/app/backend/admin_extra.py` (already in place) is now fully consumed by the Admin Mission Control page (`/app/frontend/src/pages/Admin.jsx`) via three new components: **AdminTours** (full Create/Edit/Delete + PIN copy), **AdminRewards** (Partner goodies CRUD — discount + goodie templates), **AdminAdvisories** (Road advisories CRUD that publishes live into the Info Center → Roads & Transport tab without a code deploy). The Bookings tab gains a one-click **Export CSV** button that downloads `andeor-bookings-YYYY-MM-DD.csv` with 13 hydrated columns (booking_id, user_id, user_email, user_name, tour_id, tour_name, tour_category, tour_region, date, status, created_at, completed_at, xp_awarded). Active-booking guard prevents deleting a tour with live bookings. Tested by testing_agent_v3: **21/21 pytest backend pass + all frontend admin flows green** (iter14 report).
- ✅ **Ti Dodo Saga voice-line on Saga Claim** — the one-line TTS clip the user requested ("Mo finn fyer ou — Mauritius is in your bones now.") now plays exactly once during the SagaConfetti burst. New backend endpoint `GET /api/codex/saga-voice?saga_id={mq-id}` (public, cached MP3, OpenAI TTS `tts-1-hd` voice `fable`) serves a 4 per-saga variants + a generic fallback. Wired into `CharacterSheet.jsx` via a hidden `<audio>` ref that auto-plays on sheet open when the player has a saga at 100% & unclaimed. `AvatarHud.refreshUnread` now tracks `ready_saga_id` (first ready saga) so the voice line is saga-specific. Respects global mute toggle. Self-tested end-to-end: created a saga-ready player, opened sheet → network call `/codex/saga-voice?saga_id=mq-wayfarer` fired + `<audio>` readyState=4 paused=false volume=0.85. Backend pytest **8/8 PASS** (`/app/backend/tests/test_iter15_saga_voice.py`).
