...
import {
  ChevronLeft, MapPin, BookOpen, Utensils, Landmark, Church, Castle, Trophy, Sparkles, ChevronRight,
} from "lucide-react";
  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        {/* Top bar */}
        <div className="absolute top-0 inset-x-0 z-10 p-5 lg:p-7 flex justify-between items-start gap-3">
          <button
            onClick={() => { playClick(); onClose(); }}
            data-testid="port-louis-back"
            className="inline-flex items-center gap-2 rounded-full bg-sand-100/95 backdrop-blur px-4 py-2 text-sm font-bold tracking-wider text-ink-900 hover:bg-white shadow-clay transition-colors"
          >
            <ChevronLeft className="w-4 h-4" /> Leave Port Louis
          </button>
        ...
        {focusedQuest && (
          <motion.div
            ...
            className="absolute top-20 left-1/2 -translate-x-1/2 z-10 rounded-full px-4 py-2 text-sand-100 text-xs font-bold tracking-[0.2em] uppercase flex items-center gap-2 shadow-lift"
        ...
        {cityTours.map((t, i) => {
          ...
          return (
            <motion.button
              key={t.tour_id}
              onClick={() => { playSelect(); setConfirm(t); }}
